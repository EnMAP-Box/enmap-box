...
class RasterSplitter(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        html = (
            ''
            '<p>This algorithm splits a spectral imaging raster and a corresponding classification label raster '
            'into smaller image tiles(chips). Label raster must be given in Integer format.  '
            'The value zero is fixed as unclassified. Additionally, if the number of class-labels on image '
            'tiles falls below a user-specified percentage threshold, the tiles are skipped and '
            'excluded from further processing.'
            '</p>'
            '<h3>Raster image</h3>'
            '<p>Input image raster in form of TIFF file. Accepts any data ranges. '
            'If No data value in raster defined, it is used to automatically set corresponindg labels '
            'in the label raster image to 0.'
            '</p>'
            '<h3>Raster labels </h3>'
            '<p>Input label raster in form of TIFF file. Labels are expected to be Integers in range 0-255. '
            'The value 0 is reserved for no-data class or unclassified. If label raster has zero values they will '
            'be interpreted as unclassified and masked & ignored throughout the Specdeepmap workflow. </p>'
            '<h3>tile size X </h3>'
            ...
            '<p>If the defined minimum percentage of class-labels per image tile is not reached, '
            'the tile will be skipped and excluded from further processing. The fixed label value indicating no data '
            'or unclassified is 0. The no data label 0 will continuously be masked during training and prediction, '
            'so the model will ignore the all label pixels with the value 0. '
            '</p>'
            '<h3>Output folder</h3>'
            '<p>Location of output folder. In the output folder two subfolder will be created. '
            'One images and one labels. Corresponding images and labels tiles have same name.'
            '</p>')
        return html
