...
class DL_Trainer(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        ...
        html = '' \
               ...
               '<p>The main architecture for the deep learning model can be defined with this variable. Options are Unet, Unet++ and DeeplabV3+, SegFormer, these models need to be combine with a specified backbone. Also 2D-Justo-UNet-Simple can be used as architecture, but this architecture cant be used with backbone.</p>' \
               '<h3>Deep learning model: backbone</h3>' \
               '<p> The model can be built with several backbones from pytorch segmentation models library, e.g. ResNet 50, ResNeXt, EfficientNet, by passing the correct model encoder name as string into the parameter ( so copy name and paste) e.g. by passing the string resnet50 for ResNet-50 or the string efficientnet-b0  for a small EffitientNet . For available options look into pytorch segmentation models libary here https://smp.readthedocs.io/en/latest/encoders.html . Also a range of Timm Encoder can also be used, like advanced models ConvNext or Swin-Transformer. For using these models also past the encoder string into the parameter of the model with the prefix  tu- follwed by timm-encoder name  e.g. tu-convnext_tiny  for a small ConvNext </p>' \
