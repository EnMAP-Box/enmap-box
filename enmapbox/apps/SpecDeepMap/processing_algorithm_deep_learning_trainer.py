...
class DL_Trainer(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        html = (
            ''
            '<p>This algorithm trains a deep learning model specifically a U-Net, U-Net++ or DeeplabV3+, '
            'SegFormer model with a definable backbone for a semantic segmentation task. '
            'For Training the model a Cross-entropy loss or balanced Cross-entropy loss is used. '
            'The performance metric, which is used for training is Intersection over Union.'
            '</p>'
            '<h3>Input folder (Training and validation dataset)</h3>'
            '<p>The input folder should be the one generated by the Dataset Maker, it needs to have a train, '
            'validation and test dataset csv file, as well as a summary csv file and '
            'optionally also a normalization csv file.'
            '</p>'
            '<h3>Deep learning model: architecture</h3>'
            '<p>The main architecture for the deep learning model can be defined with this variable. '
            'Options are U-Net, U-Net++ and DeeplabV3+, SegFormer, these models need to be combine with a '
            'specified backbone. Also 2D-Justo-UNet-Simple can be used as architecture, '
            'but this architecture can not be used with backbone.</p>'
            '<h3>Deep learning model: backbone</h3>'
            '<p> The model can be built with several backbones from pytorch segmentation models library, '
            'e.g. ResNet 50, ResNeXt, EfficientNet, by passing the correct model encoder name as string '
            'into the parameter (so copy name and paste) e.g. by passing the string resnet50 for ResNet-50 or '
            'the string efficientnet-b0  for a small EffitientNet. For available options look into pytorch '
            'segmentation models libary here https://smp.readthedocs.io/en/latest/encoders.html . '
            'Also a range of Timm Encoder can also be used, like advanced models ConvNext or Swin-Transformer. '
            'For using these models also past the encoder string into the parameter of the model with the prefix  '
            'tu- followed by timm-encoder name  e.g. tu-convnext_tiny  for a small ConvNext. '
            'You can find list of available Timm encoders here: '
            'https://smp.readthedocs.io/en/latest/encoders_timm.html '
            '</p>'
            '<h3>Load pretrained weights </h3>'
            '<p>Here a user can chose weights for model initialization. The options are None for training from '
            'scratch and pretrained imagenet weights for RGB images which can be extended to more spectral bands. '
            'The input data need to be structured in bgr + additional channels format for imagenet weights use as '
            'well as requires data normalization. Further pretrained weights for  Sentinel-2 Top of Atmosphere '
            'are provided for ResNet-18 and ResNet-50. These weights stem from foundation model training from '
            'SSL4EO-S12 by Wang et al. 2023. The automatic preprocessing for these weights is scaling the input '
            'data by 10000, any other scaling or normalization will be ignored if these weights are chosen.'
            '</p>'
            '<h3>Load model from path</h3>'
            '<p>Load model from path for continuing training or to initialize a model. Restriction is that model must '
            'be compatible with training scheme and input data type.'
            '</p>'
            '<h3>Freeze backbone </h3>'
            '<p>This freezes the weight of the backbone for training. This is a routine step for '
            'transferlearning where in the first step a model is trained on new classes with a frozen backbone '
            'and in a second training step it is fine-tuned with an unfrozen backbone. To achieve this here, '
            'train first with frozen backbone and then load trained model again and train with unfrozen backbone.'
            '</p>'
            '<h3>Data augmentation (random flip ) </h3>'
            '<p>This apply data augmentation. Random vertical and horizontal flip. Each augmentation has a '
            'probability of occurring of 50 %. This data augmentation is happening on the fly and '
            'prevents overfitting of the model.'
            '</p>'
            '<h3>Early stopping</h3>'
            '<p>This stops the model when validation loss is not improving for 50 epochs.</p>'
            '<h3>Balanced Training using Class Weights</h3>'
            '<p>This parameter enables balanced training, for this the precomputed class weights based on the '
            'training dataset are used, which are listed in the summary csv file.'
            '</p>'
            '<h3>Data Normalization</h3>'
            '<p>This parameter normalizes the image data with mean and std. per channel. To use this parameter '
            'the normalization statistic had to be created using the Dataset Maker.'
            '</p>'
            '<h3>Batch size</h3>'
            '<p>This defines the number of images which are processed in batches. </p>'
            '<h3>Epochs</h3>'
            '<p>This defines the number of Epochs which are used to train a model. One epochs means the model is '
            'trained once on the whole training dataset. Epochs are 0 indexed so the first training epoch is epoch-0. '
            '</p>'
            '<h3>Learning rate</h3>'
            '<p>This defines the Learning rate for the Adam optimizer of the model. Learning rate is decreasing '
            'during training using CosineAnnealingLR using defined epochs value as T_max value, so lr decreases '
            'steadily towards near zero during whole training run (min lr: 1e-6). '
            ...
            '<p>If activated this parameter runs a learning rate finder, meaning it test 100 learning rates '
            'before training and picks the one with the most stable learning for training. If chosen this '
            'overwrites the manual defined learning rate. The found and implemented learning-rate is printed '
            'to log interface. '
            '</p>'
            '<h3>Number of workers</h3>'
            '<p>This defines number of cpus used for data loading and augmentation and supports your training speed.'
            '</p>'
            ...
            '<p> Defines how many models should be saved. -1 means each epoch a model is saved. '
            'Other integer define a limited number of models.</p>'
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        feedback.pushInfo(
            "If you use pretrained weights of specific backbone for the first time, "
            "they will be downloaded before the training starts.")
