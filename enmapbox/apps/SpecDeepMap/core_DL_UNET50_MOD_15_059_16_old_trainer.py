...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if self.remove == 'Yes':
            mask_array = mask.ReadAsArray().astype(np.float32)
            mask = torch.as_tensor(mask_array, dtype=torch.int64) ### last edit
            ...
            mask_array = torch.nn.functional.one_hot(mask,
                                                     num_classes=self.num_classes)  ## added +1 one to also encode no data class
...
class MyModel(L.LightningModule):
    ...
    def training_step(self, batch: dict, batch_idx: int):
        ...
        if self.classes == 1:
            train_loss = torch.nn.BCEWithLogitsLoss(weight=self.class_weights, reduction="none")(preds, y).mean()  # .mean()
        else:
            train_loss = torch.nn.CrossEntropyLoss(weight=self.class_weights, reduction="none")(preds, y).mean()  # .mean()
    ...
    def validation_step(self, batch: dict, batch_idx: int):
        ...
        if self.classes == 1:
            val_loss = torch.nn.BCEWithLogitsLoss(weight=self.class_weights, reduction="none")(preds, y).mean() # .mean()
        else:
            val_loss = torch.nn.CrossEntropyLoss(weight=self.class_weights, reduction="none")(preds, y).mean()  # .mean()
