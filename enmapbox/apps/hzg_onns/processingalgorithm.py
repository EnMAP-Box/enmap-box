...
class OnnsProcessingAlgorithm(QgsProcessingAlgorithm):
    ...
    SENSORS_USER = ['OLCI (Ocean and Land Colour Instrument, Europe)',
                    "MERIS (Medium Resolution Imaging Spectrometer, Europe)",
                    "VIIRS (Visible Infrared Imaging Radiometer Suite, USA)",
                    "MODIS (Moderate Resolution Imaging Spectrometer, USA)",
                    "EnMAP (Environmental Mapping and Analysis Program, Germany)",
                    "GOCI2 (Geostationary Ocean Color Imager, South Korea)", "OCM2 (Ocean Colour Monitor, India)",
                    "PACE (Ocean Color Instrument of the Plankton, Aerosol, Cloud, ocean Ecosystem mission, USA)",
                    "SeaWiFS (Sea-Viewing Wide Field-of-View Sensor, USA)",
                    "SGLI (Second-Generation Global Imager, Japan)"]
    ADAPT = ['No band shifting', 'First OLCI band (400 nm) is adapted (replaced)',
             'All OLCI bands are adapted (replaced)']
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        try:
            ...
            cmd, output = onns(inputfile=self.parameterAsFile(parameters, self.P_FILE, context),
                               ...
                               ac=self.parameterAsEnum(parameters, self.P_AC, context) + 1,
    ...
    def shortHelpString(self):
        ...
        html = (
            ...
            ''
            '<h3>Band-shifting</h3>'
            '<p>The algorithm requires input at 11 OLCI bands, namely remote-sensing reflectances at 400, 412.5, '
            '442.5, 490, 510, 560, 620, 665, 755, 777.5 and 865 nm. A spectral band-shifting procedure is implemented, '
            'which allows exploitation of atmospherically corrected input from other ocean colour missions too ['
            '<a href="https://doi.org/10.1364/OE.27.00A707">Hieronymi, 2019</a>'
            ']. In case of OLCI data, one has three options: no band-shifting or replacing reflectance input at only '
            'one or all spectral bands, e.g. in case of faulty atmospheric correction. In case of MERIS, options 2 '
            'and 3 are allowed. Complete band-shifting (option 3) must be applied for all other sensors. </p>'
            ''
            '<h3>Atmospheric correction</h3>'
            '<p>Results of the previously calculated atmospheric correction may vary significantly depending on the '
            'water type, which transfers to the ONNS products. For OLCI, three atmospheric correction methods are '
            'applicable, namely the "C2R" (Case-2 Regional, standard method for ONNS application) by ['
            '<a href="https://www.brockmann-consult.de/wp-content/uploads/2017/11/sco1_12brockmann.pdf">Brockmann '
            'et al., 2016</a>], "Polymer" by ['
            ...
            '<p>The minimum output contains 12 ocean colour products with an estimate of their associated '
            'uncertainties, '
