...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-sensor', action='store', help='Define used sensor - currently atmospheric corrected data processing of: '
    ...
    parser.add_argument(
        '-adapt', action='store', help='Option for band adaptation: 0 = no band adaptation (default), 1 = only band '
                                       '400nm is adapted (replaced), 2 = all bands are adapted (replaced) from MERIS '
                                       'input'
    )
    parser.add_argument(
        '-ac', action='store', help='Define used atmospheric correction: 0 = InSitu (no AC applied, txt data), '
                                    '1 = C2R (default), 2 = POLYMER, 3 = IPF, 4 = FUB'
    )
    parser.add_argument(
        '-osize', action='store', help='Define output size: 0 = standard product output (12 products + uncertainty) '
                                       '(default), 1 = extended processor output (+ Rrs, total IOPs, Dominance, etc), '
                                       '2 = excessive processor output incl. OWT details'
    )
    parser.add_argument(
        '-txt_header', action='store', help='For in situ data (txt format): 0 = no header, 1 = header line (default), '
                                            'n = number of header lines'
    )
    parser.add_argument(
        '-txt_ID', action='store', help='For in situ data (txt format): 0 = no line IDs (e.g. station number), '
                                        '1 = first column with ID (default)'
    )
    parser.add_argument(
        '-txt_columns', '--list', nargs='+', action='store', help='For in situ data (txt format) and depending on '
