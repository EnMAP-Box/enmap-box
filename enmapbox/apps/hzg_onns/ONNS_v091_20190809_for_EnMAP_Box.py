...
def prepare_processor_input(inpath, infile):
    ...
    if ac == 1:
        ...
        # total_invalid_mask     = cloud +   invalid + cloud_risk + rtosa_oos + land_1   # MH: inland waters are not
        # masked out
...
def prepare_processor_input_MERIS(inpath, infile):
    ...
    if ac == 1:
        ...
        # in_varnames    = ('rhow_1', 'rhow_2', 'rhow_3', 'rhow_4', 'rhow_5', 'rhow_6', 'rhow_7', 'rhow_10','rhow_12',
        # 'rhow_13')
...
def classify_clustering_fuzzy(output_size, Rrs_ONNS,
                              path_to_classes="./classification/preselect_reducedHL_20161006H1203"):
    ##
    # Read scene or data.
    ...
    matching = [s for s in test if
                "weights" in s]  # DM: only identify, whether data has to be scaled! Weights are not applied to the
    # data, but in the calculation of the distances!
    ...
    #     plt.savefig(os.path.join(outpath, 'maxMembDistr_' + class_name + "_" + infile + '.png'), dpi = 500)
    ...
    #
    #     plt.savefig(os.path.join(outpath, 'maxMemb_' + class_name + "_" + infile + '.png'), dpi = 500)
    # save the figure to file
    #     plt.close()
    #
    #
    ...
    #     cb          = plt.colorbar(img, shrink = 0.8)
    #
    #     plt.savefig(os.path.join(outpath, 'TotalMemb_' + class_name + "_" + infile + '.png'), dpi = 500)
    # save the figure to file
    #     plt.close()
    #
    ...
    for i in range(m2.shape[1]):
        m2[:, i] = m2[:, i] / total_membership  # MH: if there is any membership (total_membership > 0),
        # than new total_membership is 1
    ...
    return (m, m2, total_membership, maxMemb, flag_nonclassify, flag_lowmember_01, flag_lowmember_03,
            flag_lowmember_05, flag_lowmember_09, flag_ONNS_valid)
...
def merge_products(total_membership, total_out_weighted, total_out_bNN, Case):
    ...
    for i in range(len(flag_Case_bNN)):
        ...
        elif (Case[i] is False and total_membership[i] < 0.3):
...
def save_results(outname, maxMemb, m, m2, total_membership, total_out_weighted, total_out_bNN, total_out_merged,
    ...
    out.info = ('ONNS (OLCI Neural Network Swarm) is a multi-water algorithm, specially designed to retrieve water '
                'quality parameters from SENTINEL-3 OLCI satellite data. In theory, the algorithm is applicable '
                'for all natural waters, from clearest oceanic waters to very turbid coastal or highly absorbing '
                'waters. Fuzzy logic optical water type classification is applied in conjunction with a set of '
                'specific neural networks.')
    out.version = version
    out.version_release = release

    out.originator = 'Martin Hieronymi'
    out.originator_contributer = ('Dagmar Mueller, Wolfgang Schoenfeld, Helmut Schiller, Daniel Behr, Hajo Krasemann '
                                  'et al.')
    ...
    out.reference_2 = ('Hieronymi, M. (2019). Spectral band adaptation of ocean color sensors for applicability of '
                       'the multi-water biogeo-optical algorithm ONNS. Optics Express, 27(12), A707-A724. '
                       'https://doi.org/10.1364/OE.27.00A707')
    ...
    if (output_size > 0):
        ...
        test.long_name = ('Level_2 flag for insufficient memberships in optical water types (OWT). It applies if '
                          'the fuzzy logic OWT classification cannot attribute weights above 0.1 to any OWT.')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('Level_2 flag for non-dominant optical water types (OWT). '
                          'It applies if the fuzzy logic OWT classification cannot find a dominant OWT and '
                          'only attributes weights below 0.5 to single OWTs.')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('Level_2 mask that shows where ONNS in-water products are valid. '
                          'It applies if the fuzzy logic OWT classification attributes high weights.')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('Level_2 flag for application of background NNs for merged OC products in case '
                          'of low OWT total memberships (of < 0.3). 0 = OWT classification valid. '
                          '1 = "Case-1 water" background NN applied. 2 = "Case-2 water" background NN applied.')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('Shows the optical water type (OWT) with maximum membership. '
                          'The applied fuzzy logic OWT classification distinguishes between 13 OWTs.')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('Shows the total membership of all optical water types (OWT). '
                          'The applied fuzzy logic OWT classification distinguishes between 13 OWTs.')
        test.coordinates = 'lat lon'
    ...
    if (output_size > 0):
        ...
        test.long_name = ('(ONNS) Total scattering coefficient at 440 nm, including phytoplankton, minerals, '
                          'and water (0.0042557 m-1 at 20degC/30PSU)')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('(ONNS) Dominance of water constituents in terms of the absorption coefficient at 440 nm '
                          '(without water absorption, which is 0.0064 m-1 at 20degC/30PSU). 0 = mixed, '
                          '1 = phytoplankton-dominated absorption, 2 = CDOM-dominated absorption, '
                          '3 = sediment-dominated absorption.')
        test.coordinates = 'lat lon'
        ...
        a = 5.5 * total_out_merged[:, 12] + 2.7 * total_out_merged[
            :, 4]  # MH:  5.5* bbp510 + 2.7* ap440 according RR best and phyiscal fit
        b = np.zeros(valid.shape)
        b[valid] = a
        b[~valid] = np.nan
        ...
        test.long_name = ('(ONNS) Total particulate organic carbon (POC) related to Phytoplankton and TSM. '
                          'POC = POC_TSM_2 + POC_Phy_1. According HZG in situ data --> POC = 5.5* bbp510 + 2.7* ap440 ')
        test.coordinates = 'lat lon'
...
def save_results_txt(
        outname, outname_1, outname_2, outname_3, maxMemb, m, m2, total_membership, total_out_weighted,
    ...
    if (output_size == 0):
        ...
        np.savetxt(f, X=data, delimiter='\t',
                   fmt='%11.0f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.0f %11.3f %11.3f %11.3f '
                       '%11.3f')
        ...
        Header = [
            '%        ID    Chl_ONNS   CDOM_ONNS    ISM_ONNS  ag440_ONNS  ap440_ONNS  am440_ONNS  bp440_ONNS  '
            'bm440_ONNS     FU_ONNS  Kd490_ONNS  Ku490_ONNS adg412_ONNS bbp510_ONNS     atot440     btot440   '
            'dominance   POC_TSM_1   POC_TSM_2   POC_Phy_1   POC_Phy_2    POC_ONNS    DOC_ONNS     MaxMemb     '
            'Rrs_max']
        ...
        np.savetxt(f, X=data, delimiter='\t',
                   fmt='%11.0f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.0f %11.3f %11.3f %11.3f '
                       '%11.3f %11.3f %11.3f %11.0f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.0f %11.1f')

    f.close()

    #
    # One file with product uncertainties --> maybe NaN because not applied to merged ONNS products, only from OWT
    # classification
...
if __name__ == '__main__':
    ...
    parser.add_argument('-sensor', action='store',
                        help='Define used sensor - currently atmospheric corrected data processing of: "OLCI" '
                             '(default; C2R, Pol, IPF, FUB), "MERIS" (Pol), "VIIRS" (Pol), or "MODIS" (Pol); '
                             'alternatively in situ data (.txt) at wavelengths of: "EnMAP", "GOCI2", "OCM2", '
                             '"PACE", "SeaWiFS", "SeaWiFS_OCCCI", or "SGLI"')
    parser.add_argument('-adapt', action='store',
                        help='Option for band adaptation: 0 = no band adaptation (default), 1 = only band 400nm '
                             'is adapted (replaced), 2 = all bands are adapted (replaced) from MERIS input')
    ...
    parser.add_argument('-osize', action='store',
                        help='Define output size: 0 = standard product output (12 products + uncertainty) (default), '
                             '1 = extended processor output (+ Rrs, total IOPs, Dominance, etc), '
                             '2 = excessive processor output incl. OWT details')
    parser.add_argument('-txt_header', action='store',
                        help='For in situ data (txt format): 0 = no header, 1 = header line (default), '
                             'n = number of header lines')
    parser.add_argument('-txt_ID', action='store',
                        help='For in situ data (txt format): 0 = no line IDs (e.g. station number), '
                             '1 = first column with ID (default)')
    parser.add_argument('-txt_columns', '--list', nargs='+', action='store',
                        help='For in situ data (txt format) and depending on sensor: Specification of used '
                             'columns, e.g. [0, 1, 2, 3, 4, 5, 6, 7, 11, 15, 16] = columns of OLCI bands used for ONNS',
                        required=True)
    ...
    if args.sensor == 'OLCI':
        ...
        if args.outfile or args.ac == '0':  # MH: if output file name is provided...
            ...
            if args.ac == '1':  # C2R
                ...
                print(
                    'Error: Provide atmospheric corrected (Level-2) or in situ reflectance data at OLCI '
                    'bands (C2R, POL, IPF, FUB or txt). '
                )
                print(
            ...
            if args.ac == '1':  # C2R
                ...
                if test > -1:
                    ...
                    print(
                        'Error: In case of IPF baseline corrected data, provide directory with .nc data, i.e. '
                        '".SEN3" directory '
                    )
                    sys.exit()
    ...
    if args.ac == '0':
        ...
        if args.txt_ID is not None:
            ...
            print(
                'Error: Please provide -txt_ID --> 0 = no line IDs (e.g. station number), 1 = first column with '
                'ID (default)'
            )
        ...
        if args.txt_header is not None:
            ...
            print(
                'Error: Please provide -txt_header --> 0 = no header, 1 = header line (default), n = number of header '
                'lines'
            )
    ...
    if args.ac == '0':  # MH: In situ data alternatively: 'txt' in infile.lower():
        ...
        if (sensor == 'OLCI'):
            ...
            (Rrs_in, valid, scene_in_rhow, lat, lon, cloud, land, AC, flag_negative,
             flag_suspect) = prepare_processor_input(inpath, infile)

        elif (sensor == 'MERIS'):

            (Rrs_in, valid, scene_in_rhow, lat, lon, cloud, land, AC, flag_negative,
             flag_suspect) = prepare_processor_input_MERIS(inpath, infile)

        elif (sensor == 'MODIS'):

            (Rrs_in, valid, scene_in_rhow, lat, lon, cloud, land, AC, flag_negative,
             flag_suspect) = prepare_processor_input_MODIS(inpath, infile)

        elif (sensor == 'VIIRS'):

            (Rrs_in, valid, scene_in_rhow, lat, lon, cloud, land, AC, flag_negative,
             flag_suspect) = prepare_processor_input_VIIRS(inpath, infile)
