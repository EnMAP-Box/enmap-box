...
sdir = os.path.dirname(
    os.path.realpath(__file__))  # import nnhs from the same directory where we reside, resolving symlinks
sys.path.append(sdir)

if True:
    import nnhs

RELEASE = '20260622'  # Date of release
...
def prepare_processor_input_txt(inpath, infile, sensor, txt_ID, txt_columns, txt_header):
    ...
    # lambda_OLCI     = [400.00, 412.50, 442.50, 490.00, 510.00, 560.00, 620.00, 665.00, 673.75, 681.25, 708.75,
    # 753.75, 761.25, 764.38, 767.50, 778.75, 865.00, 885.00, 900.00, 940.00, 1020.00]
...
def prepare_processor_input_OLCI(
    inpath: str,
    infile,
    ac: int,
):
    ...
    infolder = Path(inpath) / infile
    if not infolder.is_dir():
        raise ValueError(f"Sentinel3 folder does not exist: {inpath}")
    path_geo = infolder / 'geo_coordinates.nc'
    if not path_geo.is_file():
        raise ValueError(f"Sentinel3 geo_coordinates.nc file does not exist: {path_geo}")

    nc_geo: netCDF4.Dataset = nc(str(path_geo), 'r')
    ...
    if ac == 1:
        ...
        in_varnames = ('rhow_1', 'rhow_2', 'rhow_3', 'rhow_4', 'rhow_5', 'rhow_6', 'rhow_7', 'rhow_8', 'rhow_12',
                       'rhow_16', 'rhow_17')
        ...
        lon = nc_geo.variables['longitude'][:]
        lat = nc_geo.variables['latitude'][:]
        ...
        for i in range(0, len(in_varnames)):
            ...
            flag_suspect[
                b > 10 | ~valid] = True  # MH: likely not for C2RCC valid: Mask for fill value(65535) but not
            # captured by cloud + invalid + land
    ...
    if ac == 2:
        ...
        # flag_names     = 'bitmask'
        in_varnames = ('Rw400', 'Rw412', 'Rw443', 'Rw490', 'Rw510', 'Rw560', 'Rw620', 'Rw665', 'Rw754', 'Rw779',
                       'Rw865')
    ...
    if ac == 3:
        ...
        in_varnames_1 = ('Oa01_reflectance.nc', 'Oa02_reflectance.nc', 'Oa03_reflectance.nc', 'Oa04_reflectance.nc',
        ...
        in_varnames = ('Oa01_reflectance', 'Oa02_reflectance', 'Oa03_reflectance', 'Oa04_reflectance',
                       'Oa05_reflectance', 'Oa06_reflectance', 'Oa07_reflectance', 'Oa08_reflectance',
                       'Oa12_reflectance', 'Oa16_reflectance', 'Oa17_reflectance')
        ...
        for i in range(0, len(in_varnames)):
            ...
            if i == 0:
                ...
                flag_suspect[
                    b > 10 | ~valid] = True  # MH: Mask for fill value(65535) but not captured by cloud + invalid + land
...
def prepare_processor_input_MERIS(
    inpath,
    infile,
    ac: int
):
    ...
    if ac == 1:
        ...
        in_varnames = ('rhown_1', 'rhown_2', 'rhown_3', 'rhown_4', 'rhown_5', 'rhown_6', 'rhown_7', 'rhown_10',
                       'rhown_12', 'rhown_13')
        ...
        #       if isinstance(l2_flags, np.ma.MaskedArray):
        # MH: bug fix for py netcdf4 version inconsistency
        #           l2_flags        = np.ma.getdata(l2_flags)
...
def prepare_processor_input_MODIS(
    inpath: str,
    infile,
    ac: int,
):
    ...
    if ac == 2:
        ...
        # MH: valid water pixel according Polymer settings = 0, "Case-2" = 1024, "Inconsistency" = 2048
        # "bitmask" description = LAND:1, CLOUD_BASE:2, L1_INVALID:4, NEGATIVE_BB:8, OUT_OF_BOUNDS:16, EXCEPTION:32,
        # THICK_AEROSOL:64, HIGH_AIR_MASS:128, EXTERNAL_MASK:512, CASE2:1024, INCONSISTENCY:2048
        ...
        # rrs             = np.empty([np.sum(valid), 11])
        # DM: Rrs or rhow ?? Important in application of NN.
        rrs = np.empty([np.sum(valid), len(in_varnames)])  # DM: Rrs or rhow ?? Important in application of NN.
...
def prepare_processor_input_VIIRS(
    inpath,
    infile,
    ac: int,
):
...
def sensor_band_adapter(
    ...
    if sensor == 'OLCI':
        ...
        if (adapt == 1):  # MH: Option that only 400 band is replaced
            ...
            for ll in range(rrs.shape[0]):
                input = np.hstack([rrs[ll, 1:]])
                out1[ll, :] = nn3.ff_nnhs(input)
...
def classify_clustering_fuzzy(
    output_size, Rrs_ONNS, path_to_classes="./classification/preselect_reducedHL_20161006H1203"
):
    ##
    # Read scene or data.
    ...
    # DM: only identify, whether data has to be scaled! Weights are not applied to the data, but in the calculation
    # of the distances!
    matching = [s for s in test if "weights" in s]
    ...
    for i in range(0, Ncluster):
        ...
        if scaled:
            ...
            dist[:, i] = np.apply_along_axis(lambda x: np.sum(weights * (x - new_means[0:(Nvar), i]) ** 2 / diagVI), 1,
                                             d_all[:, 0:(Nvar)])  # pseudo-mahalanobis with weights!

        else:

            dist[:, i] = np.apply_along_axis(lambda x: mahalanobis(u=x, v=new_means[0:(Nvar), i], VI=VI) ** 2, 1,
                                             d_all[:, 0:(Nvar)])
    ...
    for i in range(m2.shape[1]):
        # MH: if there is any membership (total_membership > 0), than new total_membership is 1
        m2[:, i] = m2[:, i] / total_membership
    ...
    # flag_ONNS_valid[flag_nonclassify | flag_lowmember_09]  = False
    # MH: pixel where all ONNS results are valid
    flag_ONNS_valid[flag_nonclassify | flag_lowmember_03] = False  # MH: pixel where all ONNS results are valid

    return (m, m2, total_membership, maxMemb, flag_nonclassify, flag_lowmember_01, flag_lowmember_03, flag_lowmember_05,
            flag_lowmember_09, flag_ONNS_valid)
...
def ONNS(path_to_NN, Rrs_ONNS, m2):
    ...
    for nntype in NNtype:
        ...
        for NNname, i in zip(netnames, list(range(13))):
            ...
            for ll in range(np.sum(ID)):
                input = np.hstack([a[ll, :]])
                out1[ll, :] = nn2.ff_nnhs(input)
...
def background_NN(path_to_NN, Rrs_ONNS):  # MH: alternative NNs that are applied in case of low memberships
    ...
    for ll in range(len(ID)):
        input = np.hstack([rrs[ID[ll], :]])

        out1[ID[ll], :] = nn1.ff_nnhs(input)
...
def apply_BIAS_NN(
    ...
    Variables = ['Chl', 'ISM', 'a_g_440', 'a_p_440', 'a_m_440', 'b_p_440', 'b_m_440', 'FU', 'K_d_490', 'K_u_490',
                 'a_dg_412', 'b_bp_510']
    # Variables_text = ['Chl_uncertainty', 'ISM_uncertainty', 'a_g_440_uncertainty', 'a_p_440_uncertainty',
    # 'a_m_440_uncertainty', 'b_p_440_uncertainty', 'b_m_440_uncertainty', 'FU_uncertainty', 'K_d_490_uncertainty',
    # 'K_u_490_uncertainty', 'a_dg_412_uncertainty', 'b_bp_510_uncertainty']
...
def merge_products(total_membership, total_out_weighted, total_out_bNN, Case):
    ...
    for i in range(len(flag_Case_bNN)):
        ...
        elif not Case[i] and total_membership[i] < 0.3:
...
def save_results(
    ...
    total_out_weighted: float,
    total_out_bNN,
    total_out_merged,
    total_BIAS_out_weighted,
    flag_nonclassify: int,
    flag_lowmember_01: int,
    flag_lowmember_03: int,
    flag_lowmember_05,
    flag_lowmember_09,
    flag_ONNS_valid,
    Rrs_ONNS,
    ...
    out.info = ('ONNS (OLCI Neural Network Swarm) is a multi-water algorithm, specially designed to retrieve '
                'water quality parameters from SENTINEL-3 OLCI satellite data. In theory, the algorithm is '
                'applicable for all natural waters, from clearest oceanic waters to very turbid coastal or '
                'highly absorbing waters. Fuzzy logic optical water type classification is applied in conjunction '
                'with a set of specific neural networks.')
    out.version = version
    out.version_release = RELEASE

    out.originator = 'Martin Hieronymi'
    out.originator_contributer = ('Dagmar Mueller, Wolfgang Schoenfeld, Helmut Schiller, Daniel Behr, '
                                  'Hajo Krasemann et al.')
    out.originator_institution = 'Helmholtz-Zentrum Geesthacht, Germany'
    out.contact = 'martin.hieronymi@hzg.de'

    out.reference_1 = ('Hieronymi, M., Mueller, D., & Doerffer, R. (2017). The OLCI Neural Network Swarm (ONNS): '
                       'A bio-geo-optical algorithm for open ocean and coastal waters. Frontiers in Marine Science, '
                       '4, 140. https://doi.org/10.3389/fmars.2017.00140')
    out.reference_2 = ('Hieronymi, M. (2019). Spectral band adaptation of ocean color sensors for applicability '
                       'of the multi-water biogeo-optical algorithm ONNS. Optics Express, 27(12), '
                       'A707-A724. https://doi.org/10.1364/OE.27.00A707')
    ...
    out.output_size_info = ('Option for data output size: 0 = standard product output (12 products + uncertainty) '
                            '(default), 1 = extended processor output (+ Rrs, total IOPs, Dominance, etc), '
                            '2 = excessive processor output incl. OWT details')
    ...
    if (output_size > 0):
        ...
        test.long_name = ('Level_2 flag for implausible AC output. It applies if any value from the remote sensing '
                          'reflectance spectrum, which is delivered by the atmospheric correction, is negative.')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('Level_2 flag for implausible AC output. It applies if any value from the remote sensing '
                          'reflectance spectrum, which is delivered by the atmospheric correction, is above 10.')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('Level_2 flag for non-classifiable reflectance spectrum. It applies if the fuzzy logic '
                          'optical water type (OWT) classification cannot attribute weights above 0.0001 to any OWT.')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('Level_2 flag for insufficient memberships in optical water types (OWT). It applies if the '
                          'fuzzy logic OWT classification cannot attribute weights above 0.1 to any OWT.')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('Level_2 flag for low memberships in optical water types (OWT). It applies if the fuzzy '
                          'logic OWT classification cannot attribute weights above 0.3 to any OWT.')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('Level_2 flag for non-dominant optical water types (OWT). It applies if the fuzzy logic '
                          'OWT classification cannot find a dominant OWT and only attributes weights below 0.5 to '
                          'single OWTs.')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('Level_2 mask that shows where ONNS in-water products are valid. It applies if the fuzzy '
                          'logic OWT classification attributes high weights.')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('Level_2 flag for application of background NNs for merged OC products in case of low '
                          'OWT total memberships (of < 0.3). 0 = OWT classification valid. 1 = "Case-1 water" '
                          'background NN applied. 2 = "Case-2 water" background NN applied.')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('Shows the optical water type (OWT) with maximum membership. The applied fuzzy logic '
                          'OWT classification distinguishes between 13 OWTs.')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('Shows the total membership of all optical water types (OWT). The applied fuzzy logic '
                          'OWT classification distinguishes between 13 OWTs.')
        test.coordinates = 'lat lon'
        ...
        if (output_size == 2):
            ...
            for N in range(1, 14):  # 1 to 13
                ...
                if N < 10:
                    test = out.createVariable('OWT_original_weights_0' + str(N), 'f4', ('x', 'y'), zlib=True,
                                              fill_value=np.nan)
                    test2 = out.createVariable('OWT_proportional_weights_0' + str(N), 'f4', ('x', 'y'), zlib=True,
                                               fill_value=np.nan)
                else:
                    test = out.createVariable('OWT_original_weights_' + str(N), 'f4', ('x', 'y'), zlib=True,
                                              fill_value=np.nan)
                    test2 = out.createVariable('OWT_proportional_weights_' + str(N), 'f4', ('x', 'y'), zlib=True,
                                               fill_value=np.nan)
    ...
    Variables_1 = ['Chl_ONNS', 'CDOM_ONNS', 'ISM_ONNS', 'a_g_440_ONNS', 'a_p_440_ONNS', 'a_m_440_ONNS', 'b_p_440_ONNS',
                   'b_m_440_ONNS', 'FU_ONNS', 'K_d_490_ONNS', 'K_u_490_ONNS', 'a_dg_412_ONNS', 'b_bp_510_ONNS']
    Variables_bNN = ['Chl_ONNS_bNN', 'CDOM_ONNS_bNN', 'ISM_ONNS_bNN', 'a_g_440_ONNS_bNN', 'a_p_440_ONNS_bNN',
                     'a_m_440_ONNS_bNN', 'b_p_440_ONNS_bNN', 'b_m_440_ONNS_bNN', 'FU_ONNS_bNN', 'K_d_490_ONNS_bNN',
                     'K_u_490_ONNS_bNN', 'a_dg_412_ONNS_bNN', 'b_bp_510_ONNS_bNN']
    Variables_OWT = ['Chl_ONNS_OWT', 'CDOM_ONNS_OWT', 'ISM_ONNS_OWT', 'a_g_440_ONNS_OWT', 'a_p_440_ONNS_OWT',
                     'a_m_440_ONNS_OWT', 'b_p_440_ONNS_OWT', 'b_m_440_ONNS_OWT', 'FU_ONNS_OWT', 'K_d_490_ONNS_OWT',
                     'K_u_490_ONNS_OWT', 'a_dg_412_ONNS_OWT', 'b_bp_510_ONNS_OWT']
    Units_1 = ['mg.m-3', 'm-1', 'g.m-3', 'm-1', 'm-1', 'm-1', 'm-1', 'm-1', '1', 'm-1', 'm-1', 'm-1', 'm-1']
    Dscr_1 = ['(ONNS) Algal (chlorophyll) pigment concentration',
              '(ONNS) CDOM absorption coefficient at 440 nm (from concentration nets)',
              '(ONNS) Concentration of inorganic suspended matter',
              '(ONNS) CDOM absorption coefficient at 440 nm (from IOP nets)',
              '(ONNS) Absorption coefficient of phytoplankton particles at 440 nm',
              '(ONNS) Absorption coefficient of minerals at 440 nm',
              '(ONNS) Scattering coefficient of phytoplankton particles at 440 nm',
              '(ONNS) Scattering coefficient of minerals at 440 nm', '(ONNS) Forel-Ule number',
              '(ONNS) Downwelling diffuse attenuation coefficient at 490 nm',
              '(ONNS) Upwelling diffuse attenuation coefficient at 490 nm',
              '(ONNS) Absorption coefficient of detritus plus CDOM at 412 nm',
              '(ONNS) Total back-scattering coefficient of all particles at 510 nm']
    ...
    Variables_2 = ['Chl_ONNS_uncertainty', 'ISM_ONNS_uncertainty', 'a_g_440_ONNS_uncertainty',
                   ...
                   'b_m_440_ONNS_uncertainty', 'FU_ONNS_uncertainty', 'K_d_490_ONNS_uncertainty',
                   'K_u_490_ONNS_uncertainty', 'a_dg_412_ONNS_uncertainty', 'b_bp_510_ONNS_uncertainty']
    Dscr_2 = ['(ONNS) Error estimate for algal (chlorophyll) pigment concentration',
              '(ONNS) Error estimate for concentration of inorganic suspended matter',
              '(ONNS) Error estimate for CDOM absorption coefficient at 440 nm (from IOP nets)',
              '(ONNS) Error estimate for absorption coefficient of phytoplankton particles at 440 nm',
              '(ONNS) Error estimate for absorption coefficient of minerals at 440 nm',
              '(ONNS) Error estimate for scattering coefficient of phytoplankton particles at 440 nm',
              '(ONNS) Error estimate for scattering coefficient of minerals at 440 nm',
              '(ONNS) Error estimate for Forel-Ule number',
              '(ONNS) Error estimate for downwelling diffuse attenuation coefficient at 490 nm',
              '(ONNS) Error estimate for upwelling diffuse attenuation coefficient at 490 nm',
              '(ONNS) Error estimate for absorption coefficient of detritus plus CDOM at 412 nm',
              '(ONNS) Error estimate for total back-scattering coefficient of all particles at 510 nm']
    ...
    if (output_size > 0):
        ...
        atot = total_out_merged[:, 3] + total_out_merged[:, 4] + total_out_merged[
            :, 5] + aw_440_20deg_30psu  # MH: a_cdom + a_p + a_m + a_w
        b = np.zeros(valid.shape)
        b[valid] = atot
        b[~valid] = np.nan
        ...
        test.long_name = ('(ONNS) Total absorption coefficient at 440 nm, including CDOM, phytoplankton, minerals, '
                          'and water (0.0064 m-1 at 20degC/30PSU)')
        test.coordinates = 'lat lon'
        ...
        test.long_name = ('(ONNS) Total scattering coefficient at 440 nm, including phytoplankton, minerals, and water '
                          '(0.0042557 m-1 at 20degC/30PSU)')
        test.coordinates = 'lat lon'
        ...
        test.long_name = (
            '(ONNS) Dominance of water constituents in terms of the absorption coefficient at 440 nm '
            '(without water absorption, which is 0.0064 m-1 at 20degC/30PSU). 0 = mixed, '
            '1 = phytoplankton-dominated absorption, 2 = CDOM-dominated absorption, 3 = sediment-dominated absorption.'
        )
        test.coordinates = 'lat lon'
        ...
        a = 10 ** 2.525 * total_out_merged[
            :, 3] ** 0.659  # MH:  10**2.525* ag440**0.659 according Juhls et al. (2019)
        b = np.zeros(valid.shape)
        b[valid] = a
        b[~valid] = np.nan
        ...
        test.long_name = ('(ONNS) Dissolved organic carbon (DOC) related to CDOM absorption at 440 nm. According '
                          'Juhls et al. (2019) -->  DOC = 10**2.525* ag440**0.659 ')
        test.coordinates = 'lat lon'
...
# def save_results_txt(outname, outname_1, maxMemb, m, m2, total_membership, total_out_weighted,
# total_out_weighted_with_Unc, total_out_bNN, total_out_merged, total_BIAS_out_weighted, flag_nonclassify,
# flag_lowmember_01, flag_lowmember_03, flag_lowmember_05, flag_lowmember_09, flag_ONNS_valid, Rrs_ONNS,
# flag_Case_bNN, version, output_size, lambda_Rrs_max, P_ID):
def save_results_txt(outname, outname_1, outname_2, outname_3, maxMemb, m, m2, total_membership, total_out_weighted,
    ...
    if (output_size == 0):
        ...
        Header = [
            '%      P_ID    Chl_ONNS   CDOM_ONNS    ISM_ONNS  ag440_ONNS  ap440_ONNS  am440_ONNS  bp440_ONNS  '
            'bm440_ONNS     FU_ONNS  Kd490_ONNS  Ku490_ONNS adg412_ONNS bbp510_ONNS'
        ]
        ...
        np.savetxt(
            f, X=data, delimiter='\t', fmt='%11.0f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.0f '
        ...
        Header = [
            '%        ID    Chl_ONNS   CDOM_ONNS    ISM_ONNS  ag440_ONNS  ap440_ONNS  am440_ONNS  bp440_ONNS  b'
            'm440_ONNS     FU_ONNS  Kd490_ONNS  Ku490_ONNS adg412_ONNS bbp510_ONNS     atot440     btot440   '
            'dominance   POC_TSM_1   POC_TSM_2   POC_Phy_1   POC_Phy_2    POC_ONNS    DOC_ONNS     MaxMemb     Rrs_max'
        ]
        ...
        for k in range(total_out_merged.shape[0]):
            ...
            # MH: 2.3* (ap440 + am440) according RR
            data[k, 20] = 2.3 * (total_out_merged[k, 4] + total_out_merged[k, 5])
            data[k, 21] = 5.5 * total_out_merged[k, 12] + 2.7 * total_out_merged[
                k, 4]  # MH:  5.5* bbp510 + 2.7* ap440 according RR best and phyiscal fit
            data[k, 22] = 10 ** 2.532 * total_out_merged[
                k, 3] ** 0.622  # MH:  10**2.532* ag440**0.622 according Juhls et al. (in prep)
            data[k, 23] = maxMemb[k]
        ...
        np.savetxt(
            f, X=data, delimiter='\t', fmt='%11.0f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.0f '
                                           '%11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.0f %11.3f %11.3f %11.3f '
                                           '%11.3f %11.3f %11.3f %11.0f %11.1f'
        )

    f.close()

    #
    # One file with product uncertainties --> maybe NaN because not applied to merged ONNS products, only from
    # OWT classification

    f = open(outname_1, 'a')

    Header = [
        '%        ID   UChl_ONNS   UISM_ONNS Uag440_ONNS Uap440_ONNS Uam440_ONNS Ubp440_ONNS Ubm440_ONNS    '
        'UFU_ONNS UKd490_ONNS UKu490_ONNS Uad412_ONNS Ubb510_ONNS'
    ]
    ...
    np.savetxt(f, X=data, delimiter='\t',
               fmt='%11.0f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f %11.3f')
    ...
    if (output_size > 0):
        ...
        Header = [
            '%        ID      Rrs400      Rrs412      Rrs443      Rrs490      Rrs510      Rrs560      Rrs620      '
            'Rrs665      Rrs755      Rrs778      Rrs865'
        ]
        ...
        np.savetxt(f, X=data, delimiter='\t',
                   fmt='%11.0f %11.6f %11.6f %11.6f %11.6f %11.6f %11.6f %11.6f %11.6f %11.6f %11.6f %11.6f')
    ...
    if (output_size == 2):
        ...
        Header = [
            '%        ID       OWT01       OWT02       OWT03       OWT04       OWT05       OWT06       OWT07       '
            'OWT08       OWT09       OWT10       OWT11       OWT12       OWT13'
        ]
        ...
        np.savetxt(
            f, X=data, delimiter='\t', fmt='%11.0f %11.6f %11.6f %11.6f %11.6f %11.6f %11.6f %11.6f %11.6f '
...
def run_processor(cmd_string: Optional[str] = None):
    # print('Usage: ')
    # exit(0)
    ...
    parser.add_argument(
        'iprod',
        action='store',
        help='Input InSitu/C2RCC/Polymer/IPF/FUB L2 reflectance product'
    )
    parser.add_argument('--outdir', action='store', help='Output directory, default current directory')
    parser.add_argument('--outfile', action='store', help='Define name of output file')
    parser.add_argument(
        '--sensor',
        action='store',
        help='Define used sensor - currently atmospheric corrected data processing of: '
             '"OLCI" (default; C2R, Pol, IPF, FUB), "MERIS" (Pol), "VIIRS" (Pol), or "MODIS"'
             ' (Pol); alternatively in situ data (.txt) at wavelengths of: "EnMAP", '
             '"GOCI2", "OCM2", "PACE", "SeaWiFS", "SeaWiFS_OCCCI", or "SGLI"'
    )
    parser.add_argument(
        '--adapt',
        action='store',
        help='Option for band adaptation: 0 = no band adaptation (default), 1 = only band '
             '400nm is adapted (replaced), 2 = all bands are adapted (replaced) from MERIS '
             'input'
    )
    parser.add_argument(
        '--ac',
        action='store',
        type=int,
        help='Define used atmospheric correction: 0 = InSitu (no AC applied, txt data), '
             '1 = C2R (default), 2 = POLYMER, 3 = IPF, 4 = FUB'
    )
    parser.add_argument(
        '--osize',
        action='store',
        help='Define output size: 0 = standard product output (12 products + uncertainty) '
             '(default), 1 = extended processor output (+ Rrs, total IOPs, Dominance, etc), '
             '2 = excessive processor output incl. OWT details'
    )
    parser.add_argument(
        '--txt_header',
        action='store',
        help='For in situ data (txt format): 0 = no header, 1 = header line (default), '
             'n = number of header lines'
    )
    parser.add_argument(
        '--txt_ID',
        action='store',
        help='For in situ data (txt format): 0 = no line IDs (e.g. station number), '
             '1 = first column with ID (default)'
    )
    parser.add_argument(
        '--txt_columns',
        nargs='+',
        action='store',
        help='For in situ data (txt format) and depending on '
             'sensor: Specification of used columns, e.g. '
             '[0, 1, 2, 3, 4, 5, 6, 7, 11, 15, 16] = columns '
             'of OLCI bands used for ONNS',
        # required=True
    )

    if isinstance(cmd_string, str):
        parsed_list = shlex.split(cmd_string)
    ...
    if args.osize == '0':
        ...
        print(
            'Error: Define "-osize" output size: 0 = standard product output (12 products + uncertainty) (default), '
            '1 = extended processor output (+ Rrs, total IOPs, Dominance, etc), 2 = excessive processor output incl. '
            'OWT details'
        )
        sys.exit()
    ...
    if args.sensor == 'OLCI':
        ...
        if args.outfile or args.ac == 0:  # MH: if output file name is provided...
            ...
            if outfile[-3:] == '.nc':
                ...
                outname_1 = os.path.join(outpath,
                                         outfile[:-4] + '_ONNS' + version + '_uncertainty_' + str(output_size) + '.txt')
                outname_2 = os.path.join(outpath,
                                         outfile[:-4] + '_ONNS' + version + '_Rrs_' + str(output_size) + '.txt')
                outname_3 = os.path.join(outpath, outfile[:-4] + '_ONNS' + version + '_OWTmemberships_' + str(
                    output_size) + '.txt')
            ...
            if args.ac == 1:  # C2R
                ...
                print(
                    'Error: Provide atmospheric corrected (Level-2) or in situ reflectance data at OLCI bands '
                    '(C2R, POL, IPF, FUB or txt). '
                )
                print(
                    'Error: In case of IPF baseline corrected data, provide directory with .nc data, '
                    'i.e. ".SEN3" directory '
                )
                sys.exit()
                ...
                raise Exception('Error: Input file seems to be OLCI Level-1. Thus, "-ac" must be 3 for IPF.')
                # sys.exit()
    ...
    # if args.sensor != 'OLCI':
        ...
        if outfile[-3:] == '.nc':
            ...
            outname_3 = os.path.join(outpath,
                                     outfile[:-4] + '_ONNS' + version + '_OWTmemberships_' + str(output_size) + '.txt')
    ...
    if args.ac == 0:
        ...
        if args.list is not None:
            ...
            print(
                'Error: Please provide --txt_columns --> a list with specification of used columns, '
                'e.g. 0 1 2 3 4 5 6 7 11 15 16 = columns of OLCI bands used for ONNS'
            )
        ...
        if args.txt_ID is not None:
            ...
            print(
                'Error: Please provide --txt_ID --> 0 = no line IDs (e.g. station number), '
                '1 = first column with ID (default)'
            )
        ...
        if args.txt_header is not None:
            ...
            print(
                'Error: Please provide --txt_header --> 0 = no header, 1 = header line (default), '
                'n = number of header lines'
            )
    ...
    if args.ac == 0:  # MH: In situ data alternatively: 'txt' in infile.lower():
        ...
        (m, m2, total_membership, maxMemb, flag_nonclassify, flag_lowmember_01, flag_lowmember_03, flag_lowmember_05,
         flag_lowmember_09, flag_ONNS_valid) = classify_clustering_fuzzy(
            output_size, Rrs_ONNS, path_to_classes=path_to_classes
        )
        ...
        save_results_txt(outname, outname_1, outname_2, outname_3, maxMemb, m, m2, total_membership, total_out_weighted,
                         total_out_bNN, total_out_merged, total_BIAS_out_weighted, flag_nonclassify, flag_lowmember_01,
                         flag_lowmember_03, flag_lowmember_05, flag_lowmember_09, flag_ONNS_valid, Rrs_ONNS,
                         flag_Case_bNN, version, output_size, lambda_Rrs_max, P_ID)
        ...
        if (sensor == 'OLCI'):
            ...
            (Rrs_in, valid, scene_in_rhow, lat, lon, cloud, land, AC, flag_negative,
             # flag_suspect) = prepare_processor_input(inpath, infile, args.ac)
             flag_suspect) = prepare_processor_input_OLCI(inpath, infile, args.ac)

        elif (sensor == 'MERIS'):

            (Rrs_in, valid, scene_in_rhow, lat, lon, cloud, land, AC, flag_negative,
             flag_suspect) = prepare_processor_input_MERIS(inpath, infile, args.ac)

        elif (sensor == 'MODIS'):

            (Rrs_in, valid, scene_in_rhow, lat, lon, cloud, land, AC, flag_negative,
             flag_suspect) = prepare_processor_input_MODIS(inpath, infile, args.ac)

        elif (sensor == 'VIIRS'):

            (Rrs_in, valid, scene_in_rhow, lat, lon, cloud, land, AC, flag_negative,
             flag_suspect) = prepare_processor_input_VIIRS(inpath, infile, args.ac)
        ...
        save_results(outname, maxMemb, m, m2, total_membership, total_out_weighted, total_out_bNN, total_out_merged,
                     total_BIAS_out_weighted, flag_nonclassify, flag_lowmember_01, flag_lowmember_03, flag_lowmember_05,
                     flag_lowmember_09, flag_ONNS_valid, Rrs_ONNS, valid, flag_adapter_fail, flag_Case_bNN, version,
                     output_size, lambda_Rrs_max)

    print('Ready ', str(dt.now()), ' after ', str(dt.now() - start_time))


if __name__ == '__main__':
    # When executed directly from the terminal, just call the function without arguments
    run_processor()
