...
class TestPrepareRegressionDatasetFromContinuousVectorAlgorithm_2(TestCase):
    ...
    def test_styled_multitarget(self):
        ...
        self.assertListEqual(['roof', 'pavement', 'low vegetation', 'tree', 'soil', 'water'],
        ...
        self.assertEqual(384027,
                         round(QgsVectorLayer(
                             self.filename('library.gpkg')).getFeatures().__next__().geometry().asPoint().x()))
    ...
    def test_styled_singletarget(self):
        ...
        self.assertEqual(177, len(dump.features))
        self.assertEqual(['band 8 (0.460000 Micrometers)', 'band 9 (0.465000 Micrometers)'], dump.features[:2])
        self.assertListEqual(['vegetation'], [t.name for t in dump.targets])
