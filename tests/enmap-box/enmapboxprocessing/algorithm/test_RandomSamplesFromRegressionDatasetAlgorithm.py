...
class TestRandomSamplesFromRegressionDatasetAlgorithm(TestCase):
    ...
    def test_multitarget(self):
        ...
        alg.initAlgorithm()
        parameters = {
            alg.P_DATASET: regressorDumpMultiTargetSkops,  # 6 targets, 2559 samples, 177 features
            alg.P_BINS: 10,
            alg.P_N: 1,
            alg.P_SEED: 42,
        ...
        self.assertEqual((59, 177), Utils.modelLoad(parameters[alg.P_OUTPUT_DATASET])['X'].shape)
        self.assertEqual((2500, 177), Utils.modelLoad(parameters[alg.P_OUTPUT_COMPLEMENT])['X'].shape)
